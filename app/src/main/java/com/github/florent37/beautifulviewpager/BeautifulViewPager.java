package com.github.florent37.beautifulviewpager;

import android.content.Context;
import android.support.v4.view.PagerAdapter;
import android.support.v4.view.ViewPager;
import android.support.v7.widget.RecyclerView;
import android.util.AttributeSet;
import android.view.LayoutInflater;
import android.view.ViewGroup;
import android.widget.FrameLayout;
import android.widget.HorizontalScrollView;

import com.github.florent37.beautifulviewpager.recycler.HeaderHolder;
import com.github.ksoichiro.android.observablescrollview.ObservableScrollView;

import java.util.ArrayList;
import java.util.List;

/**
 * Created by florentchampigny on 07/08/15.
 */
public class BeautifulViewPager extends FrameLayout {

    protected BeautifulViewPagerAnimator animator;

    protected HorizontalScrollView headerScroll;
    protected ViewGroup headerLayout;
    protected ViewPager viewPager;

    protected List<HeaderHolder> headerHolders = new ArrayList<>();

    public BeautifulViewPager(Context context) {
        super(context);
    }

    public BeautifulViewPager(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public BeautifulViewPager(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    @Override
    protected void onAttachedToWindow() {
        super.onAttachedToWindow();
        BeautifulViewPagerBus.register(getContext(), this);
    }

    @Override
    protected void onDetachedFromWindow() {
        super.onDetachedFromWindow();
        BeautifulViewPagerBus.unregister(getContext());
    }

    public ViewPager getViewPager() {
        return viewPager;
    }

    @Override
    protected void onFinishInflate() {
        super.onFinishInflate();
        addView(LayoutInflater.from(getContext()).inflate(R.layout.beaufitul_view_pager, this, false));

        viewPager = (ViewPager) findViewById(R.id.bfp_viewPager);
        headerScroll = (HorizontalScrollView) findViewById(R.id.bfp_headerScroll);
        headerLayout = (ViewGroup) findViewById(R.id.bfp_headerLayout);

        animator = new BeautifulViewPagerAnimator(this);
    }

    public void setAdapter(PagerAdapter adapter) {
        viewPager.setAdapter(adapter);
        animator.fillHeader(adapter);
        viewPager.setOffscreenPageLimit(adapter.getCount()); //TODO remove
    }

    public void registerRecyclerView(RecyclerView recyclerView) {
        animator.registerRecyclerView(recyclerView);
    }

    public void registerScrollView(ObservableScrollView scrollView) {
        animator.registerScrollView(scrollView);
    }
}